				...
				<div className="oa-login">
					...
					<div className="app-login center-content">
						...
						<section className="login-qrcode" style={ { display: loginWithQrc ? "block" : "none" } }>
							...
							{qrCodeStat == 1 ? <div className="scan-success"><span className="ico-phone"></span><p>扫码成功<br />请在手机上<em>确认登录</em></p><span className="cancel-login" onClick={ () => this.updateQRCode() }>取消登录</span></div> : null}
							{qrCodeStat == 3 ? <div>已取消登录操作 <span onClick={ () => this.updateQRCode() }>刷新</span></div> : null}
							{isExpired ? <div>二维码已过期，请<span onClick={ () => this.updateQRCode() }>刷新</span></div> : null}
						</section>
					</div>
				</div>
			</div>
